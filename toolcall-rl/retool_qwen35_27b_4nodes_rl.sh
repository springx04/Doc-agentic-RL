...
SAVE_CKPT=${SAVE_CKPT:-/data_storage/wyj/OpenClaw-RL/ckpt/qwen35-27b-document-tool-rl/}
...
if [[ -n "${WANDB_KEY:-}" ]]; then
   WANDB_ARGS=(
      --use-wandb
      --wandb-project "${WANDB_PROJECT:-document_tool_rl}"
      --wandb-group "${WANDB_GROUP:-qwen35-27B-document-tools-4nodes}"
      --wandb-key "${WANDB_KEY}"
   )
else
   WANDB_ARGS=()
