    ...
    for world in worlds:
        ...
        try:
            for sibling in siblings:
                model = model_factory()
                trajectory = await generate_code_trajectory(default_sample(instance), model, client, DEFAULT_CODE_CONFIG, world=world, eval_script=eval_script, seed=seed, interaction_lease=sibling.lease, branch_checkpoint=sibling.checkpoint, data_source=instance.public.data_source)
                row = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = asyncio.run(run_stage_c_smoke(instance, repository_root=args.repository_root, model_factory=_default_model, group_size=args.group_size, seed=args.seed))
