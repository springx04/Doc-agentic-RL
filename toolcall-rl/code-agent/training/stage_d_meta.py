...
class PersistentCodeSession:
    ...
    async def run_task(self, instance: Any, *, repository_root: str | Path, model_client: Any, seed: int = 0) -> dict[str, Any]:
        ...
        result = await generate_code_trajectory(default_sample(instance), model_client, LocalCodeEnvClient(repository_root), DEFAULT_CODE_CONFIG, world=world, seed=seed, data_source=instance.public.data_source)
...
async def run_stage_d(instances: Iterable[Any], *, repository_root: str | Path, model_factory, seed: int = 0) -> dict[str, Any]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = asyncio.run(run_stage_d(load_instances(args.manifest), repository_root=args.repository_root, model_factory=_default_model, seed=args.seed))
